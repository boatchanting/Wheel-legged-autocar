...
HOST_CTRL_START_GPS_REPLAY = 0x03
HOST_CTRL_STOP_GPS_REPLAY = 0x04
...
def _format_host_ack_result(control_code, ack_status):
    ...
    if ack_status == HOST_ACK_REJECTED:
        ...
        if control_code == HOST_CTRL_START_GPS_REPLAY:
            return {
                "success": True,
                "executed": False,
                "msg": "小车已回传：纯GPS复刻启动被拒绝（需电机使能）",
            }
...
class Api:
    ...
    def export_mark_points_csv(self, points):
        try:
            ...
            filename = f"gps_mark_points_{time.strftime('%Y%m%d_%H%M%S')}.csv"
...
if __name__ == "__main__":
    ...
    html_path = os.path.join(current_dir, "gps_marker.html")
