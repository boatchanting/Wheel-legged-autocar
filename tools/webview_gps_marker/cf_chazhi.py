#!/usr/bin/env python3
"""Interpolate CF replay route points while preserving special-point metadata."""
...
def load_manual_launch_heading() -> float:
    sys_options_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../../code/config/sys_options.h")
    )
    text = open(sys_options_path, "r", encoding="utf-8").read()
    match = re.search(
...
def generate_header(
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write("#ifndef _CF_NAV_REPLAY_ROUTE_TABLE_H_\n")
        f.write("#define _CF_NAV_REPLAY_ROUTE_TABLE_H_\n\n")
        f.write('#include "nav_ram.h"\n\n')
        f.write("// Auto-generated by tools/webview_gps_marker/cf_chazhi.py\n")
        ...
        f.write("};\n\n")
        f.write("#endif // _CF_NAV_REPLAY_ROUTE_TABLE_H_\n")
...
def show_methods(raw_points: List[RoutePoint], methods: dict) -> None:
    ...
    fig.canvas.manager.set_window_title("CF path interpolation preview")
    axs = axs.flatten()
    ...
    for ax, (key, (name, x_res, y_res)) in zip(axs, methods.items()):
        ax.plot(x_orig, y_orig, "ro-", label="Raw control points", markersize=4, alpha=0.5, zorder=3)
        ax.plot(x_res, y_res, "b.-", markersize=3, label="Interpolated path", zorder=2)
...
def main() -> None:
    header_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../../code/navigation/cf_nav_replay_route_table.h")
    )
    raw_points, _start_heading_valid, _start_heading_deg = read_route_header(header_path)
    start_heading_valid = 0
    start_heading_deg = load_manual_launch_heading()

    if not raw_points:
        print("No route points found in CF route table.")
        return
    ...
    print("\n" + "=" * 68)
    print("CF path interpolation methods")
    ...
    if choice not in methods:
        print("Invalid choice, fallback to [4] Corner Fillet.")
        choice = "4"
    ...
    print(f"Point count written: {len(final_points)}")
    print("Special-point position, type, target yaw, and heading were preserved.")
