...
def generate_header(
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        f.write("// Auto-generated by tools/webview_nav_marker科目三/chazhi.py\n")
...
def build_methods(x_orig: np.ndarray, y_orig: np.ndarray) -> dict:
    ...
    x_lin, y_lin = fx_lin(u_fine), fy_lin(u_fine)
    methods["6"] = ("Linear", *resample_path(x_lin, y_lin, INTERPOLATE_DIST))
    ...
    try:
        ...
        methods["1"] = ("Cubic Spline", *resample_path(x_lin, y_lin, INTERPOLATE_DIST))
    ...
    try:
        ...
        methods["2"] = ("PCHIP", *resample_path(x_lin, y_lin, INTERPOLATE_DIST))
    ...
    try:
        ...
        methods["3"] = ("Akima", *resample_path(x_lin, y_lin, INTERPOLATE_DIST))
    ...
    try:
        ...
        methods["4"] = ("Corner Fillet", *resample_path(x_lin, y_lin, INTERPOLATE_DIST))
    ...
    try:
        ...
        methods["5"] = ("B-Spline", *resample_path(x_lin, y_lin, INTERPOLATE_DIST))

    return methods


def build_methods_piecewise(raw_points: List[RoutePoint]) -> dict:
    segments = []
    current_segment = []
    mode = 'INTERPOLATE'
    ...
    for p in raw_points:
        if mode == 'INTERPOLATE':
            ...
            if 1 <= p.point_type <= 9: # 遇到进入点，结束插值段
                segments.append(('INTERPOLATE', current_segment))
                current_segment = [p] 
                mode = 'KEEP'
        elif mode == 'KEEP':
            current_segment.append(p)
            if 10 <= p.point_type <= 50: # 遇到退出点，结束保留段
                segments.append(('KEEP', current_segment))
                current_segment = [p]
                mode = 'INTERPOLATE'

    if current_segment:
        segments.append((mode, current_segment))
    ...
    for i, (seg_mode, seg_points) in enumerate(segments):
        if seg_mode == 'KEEP' or len(seg_points) < 2:
            seg_x = [p.x for p in seg_points]
            seg_y = [p.y for p in seg_points]
            for m in methods_acc.values():
                if i > 0 and len(seg_x) > 0:
                    m[1].extend(seg_x[1:])
                    m[2].extend(seg_y[1:])
                else:
                    m[1].extend(seg_x)
            ...
            x_orig = np.array([p.x for p in seg_points], dtype=float)
            y_orig = np.array([p.y for p in seg_points], dtype=float)
            seg_methods = build_methods(x_orig, y_orig)
            for k in methods_acc:
                _, x_res, y_res = seg_methods[k]
                if i > 0 and len(x_res) > 0:
                    methods_acc[k][1].extend(x_res[1:])
                    methods_acc[k][2].extend(y_res[1:])
                else:
                    methods_acc[k][1].extend(x_res)
                    methods_acc[k][2].extend(y_res)

    return {
        k: (v[0], np.array(v[1]), np.array(v[2]))
        for k, v in methods_acc.items()
    }
...
def show_methods(raw_points: List[RoutePoint], methods: dict) -> None:
    ...
    fig.canvas.manager.set_window_title("路径平滑与分段插值对比 (科目三专用)")
    axs = axs.flatten()
...
def main() -> None:
    ...
    methods = build_methods_piecewise(spline_input_points)
    show_methods(spline_input_points, methods)

    print("\n" + "=" * 68)
    print("路径平滑与插值方法 (科目三 分段插值版)")
    ...
    print("特殊点的位置、类型、目标偏航和 heading 已保留 (且段内未经插值干扰)。")
