...
# 上位机 TCP 监听端口：须与下位机 WiFi 客户端配置完全一致。
HOST_PORT = 8086
...
# 上位机控制命令的应答等待上限（秒）：WiFi 延迟较高时可适当增大，过大会让界面失败反馈变慢。
...
# WiFi 全量遥测 CSV 的固定列顺序：保留原始帧和解析字段，便于联调复盘；修改时需同步下游分析脚本。
...
# 轨迹历史最大保留帧数：增大可保留更长轨迹，但会提高上位机内存占用。
MAX_HISTORY = 20000
# 单次推送给界面的新帧缓冲上限：增大可减少高频数据丢弃，但会增加界面刷新负担。
MAX_NEW_BUFFER = 4000
# 手动打点吸附首点半径（mm）：应略大于定位抖动，过大会误把邻近点判为同一首点。
