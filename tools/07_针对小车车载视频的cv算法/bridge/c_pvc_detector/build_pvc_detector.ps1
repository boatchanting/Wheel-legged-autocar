...
if ([string]::IsNullOrWhiteSpace($Output)) {
    $OutDir = Join-Path $ProjectRoot "data\bridge_white_pvc_c_run"
    $Output = Join-Path $OutDir "pvc_video_cli.exe"
}
else {
...
$DetectorC = Join-Path $ScriptDir "pvc_detector.c"
$CliC = Join-Path $ScriptDir "pvc_video_cli.c"

$Defines = @()
if ($NoTiming) {
    $Defines += "PVC_PC_ENABLE_TIMING=0"
}
else {
    $Defines += "PVC_PC_ENABLE_TIMING=1"
}

if ($NoCsv) {
    $Defines += "PVC_PC_WRITE_CSV=0"
}
else {
    $Defines += "PVC_PC_WRITE_CSV=1"
}
