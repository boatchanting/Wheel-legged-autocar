...
if ([string]::IsNullOrWhiteSpace($Output)) {
    $OutDir = Join-Path $ProjectRoot "data\bumpy_road_c_run"
    $Output = Join-Path $OutDir "bumpy_video_cli.exe"
}
else {
...
$DetectorC = Join-Path $ScriptDir "bumpy_detector.c"
$CliC = Join-Path $ScriptDir "bumpy_video_cli.c"

$Defines = @()
if ($NoTiming) {
    $Defines += "BUMPY_PC_ENABLE_TIMING=0"
}
else {
    $Defines += "BUMPY_PC_ENABLE_TIMING=1"
}

if ($NoCsv) {
    $Defines += "BUMPY_PC_WRITE_CSV=0"
}
else {
    $Defines += "BUMPY_PC_WRITE_CSV=1"
}
...
if ($Gcc) {
    ...
    & $Gcc -std=c99 -O2 -Wall -Wextra @DefineArgs $DetectorC $CliC -o $Output
...
if ($Clang) {
    $DefineArgs = $Defines | ForEach-Object { "-D$_" }
    & $Clang -std=c99 -O2 -Wall -Wextra @DefineArgs $DetectorC $CliC -o $Output
