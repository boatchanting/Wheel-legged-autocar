...
# --- 辅助函数：解析文件名获取位姿 ---
def parse_filename_pose(filename):
    """从标准文件名格式中提取 6DOF 位姿参数 (X, Y, Z, P, Yaw, Roll)。"""
    # 匹配: X{...}_Y{...}_Z{...}_P{...}_Y{...}_R{...}
    match = re.search(r"X(-?\d+)_Y(-?\d+)_Z(-?\d+)_P(-?\d+)_Y(-?\d+)_R(-?\d+)", filename)
    if not match:
        return None 
    ...
    return {
        ...
        'Zc': float(match.group(3)),
        'P': float(match.group(4)),
        'Yaw': float(match.group(5)),
        'R': float(match.group(6))
    }

# --- 辅助函数：计算世界原点在相机坐标系下的位置 ---
def calculate_true_camera_coords(pose_params):
    """
    计算世界原点 Pw(0, 0, 0) 在相机坐标系 P_c(xc, yc, zc) 中的位置。
    约定: xc (水平), yc (垂直), zc (深度, OpenGL约定负值向前)。
    """
    if pose_params is None:
        return np.array([0, 0, 0])

    Xc, Yc, Zc = pose_params['Xc'], pose_params['Yc'], pose_params['Zc']
    P, Yaw, R = pose_params['P'], pose_params['Yaw'], pose_params['R'] 
    
    # 相对平移向量 (Pw - Tc)
    T_rel = np.array([-Xc, -Yc, -Zc])

    # 旋转角度 (弧度)
    theta_x = np.deg2rad(-P) 
    theta_y = np.deg2rad(-Yaw)
    theta_z = np.deg2rad(-R)

    # 旋转矩阵 R_cw = Rz(-R) * Rx(-P) * Ry(-Y)
...
class MinefieldDetector:
    ...
    def run(self, img_path):
        ...
        pose_params = parse_filename_pose(filename)
        ...
        if not results:
            ...
            if pose_params:
                # 1. 计算世界原点的真实相机坐标 P_c (xc, yc, zc)
                true_pc = calculate_true_camera_coords(pose_params)
                
                # 2. 真实BEV坐标 (X: 水平, Y: 深度)
                X_true = true_pc[0]      # xc
                Y_true_depth = -true_pc[2] # -zc (相机深度，转化为正值)
                
                # 3. 测算值
                X_det = r['xy_meter'][0]
                Y_det_depth = r['xy_meter'][1]

                print("\n--- 坐标对比 (厘米) ---")
                print(f"  [真实值] X_true (水平): {X_true:.3f}cm, Y_true (深度): {Y_true_depth:.3f}cm")
                print(f"  [测算值] X_det (水平): {X_det:.3f}m, Y_det (深度): {Y_det_depth:.3f}m")
                
                err_x = abs(X_true - X_det*100)
                err_y = abs(Y_true_depth - Y_det_depth*100)
                print(f"  [误差]   Error_X: {err_x:.3f}cm, Error_Y: {err_y:.3f}cm")
    ...
    if not os.path.exists(json_config_path):
        ...
        for filename in os.listdir(folder_path):
            ...
            if filename.endswith('.png'):
                # 构建完整的文件路径
                test_img_path = os.path.join(folder_path, filename)
                print(f"\n--- 正在处理: {filename} ---")
                
                # run 函数现在会自行解析文件名，计算真值，并进行对比打印
