...
|序号|列名|类型|单位|说明|
|---:|---|---|---|---|
|1|`loop`|`uint32`|tick|下位机 `loop_counter`，用于帧顺序、周期与时间差分析。实际 tick 周期以固件配置为准。|
|2|`nav_x`|`float32`|mm|视觉融合导航世界坐标 X，即 `nav_vision_fusion_x`。|
|3|`nav_y`|`float32`|mm|视觉融合导航世界坐标 Y，即 `nav_vision_fusion_y`。|
|4|`vx_body`|`float32`|mm/s|车体坐标系 X 向速度，即 `inertial_nav.vx_body`。|
|5|`vy_body`|`float32`|mm/s|车体坐标系 Y 向速度，即 `inertial_nav.vy_body`。|
|6|`heading`|`float32`|°|绝对航向。仅 `IMU_CATEGORY == 3` 时发送 `heading`；其他 IMU 类型固定为 0。|
|7|`relative_yaw`|`float32`|°|相对偏航角，即 `inertial_nav.relative_yaw`。|
...
|11|`slip_flag`|`uint8`|-|惯导滑移标志 `inertial_nav.slip_flag`；`0` 表示未标记滑移，非 0 表示存在滑移/特殊工况。|
...
|24|`imu_acc_x`|`float32`|g|EKF IMU 加速度计 X 轴数据 `imu_data.acc_x`。|
|25|`imu_acc_y`|`float32`|g|EKF IMU 加速度计 Y 轴数据 `imu_data.acc_y`。|
|26|`imu_acc_z`|`float32`|g|EKF IMU 加速度计 Z 轴数据 `imu_data.acc_z`。|
|27|`imu_gyro_x`|`float32`|rad/s|EKF IMU 陀螺仪 X 轴角速度 `imu_data.gyro_x`。|
|28|`imu_gyro_y`|`float32`|rad/s|EKF IMU 陀螺仪 Y 轴角速度 `imu_data.gyro_y`。|
|29|`imu_gyro_z`|`float32`|rad/s|EKF IMU 陀螺仪 Z 轴角速度 `imu_data.gyro_z`。|
|30|`imu_grav_x`|`float32`|g|EKF 估计的重力单位向量 X 分量 `imu_data.grav_x`。|
|31|`imu_grav_y`|`float32`|g|EKF 估计的重力单位向量 Y 分量 `imu_data.grav_y`。|
|32|`imu_grav_z`|`float32`|g|EKF 估计的重力单位向量 Z 分量 `imu_data.grav_z`。|
