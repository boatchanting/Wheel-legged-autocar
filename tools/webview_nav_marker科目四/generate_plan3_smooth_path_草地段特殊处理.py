...
def find_sample_index_for_marker(samples: list[PathSample], marker: Marker) -> int:
    """Return the index of the sample closest to the given marker."""
    best_idx = 0
    best_dist = float("inf")
    for i, sample in enumerate(samples):
        d = math.hypot(sample.x - marker.x, sample.y - marker.y)
        if d < best_dist:
            best_dist = d
            best_idx = i
    if best_dist > 5.0:  # 5 mm tolerance
        raise ValueError(f"在采样路径中找不到标记点 order={marker.order} 的近似位置（最近距离 {best_dist:.1f} mm）。")
    return best_idx
...
def main() -> int:
    ...
    # 交互式输入两个点索引（order）
    try:
        start_idx = int(input("请输入 START_GRASS 点的 index (order): ").strip())
        end_idx = int(input("请输入 END_GRASS 点的 index (order): ").strip())
    except (ValueError, EOFError) as e:
        print(f"输入无效: {e}", file=sys.stderr)
        raise SystemExit(1)

    # 在 markers 中查找对应 order 的 Marker
    order_to_marker = {m.order: m for m in markers}
    if start_idx not in order_to_marker:
        raise ValueError(f"START_GRASS index={start_idx} 在标记点中不存在。")
    if end_idx not in order_to_marker:
        raise ValueError(f"END_GRASS index={end_idx} 在标记点中不存在。")
    start_marker = order_to_marker[start_idx]
    end_marker = order_to_marker[end_idx]

    samples, effective_sample_step_mm = generate_path_with_point_cap(markers)

    # 定位采样索引
    start_sample_idx = find_sample_index_for_marker(samples, start_marker)
    end_sample_idx = find_sample_index_for_marker(samples, end_marker)
    if start_sample_idx > end_sample_idx:
        start_sample_idx, end_sample_idx = end_sample_idx, start_sample_idx
    print(f"START_GRASS (order={start_idx}) -> 采样点 {start_sample_idx}, END_GRASS (order={end_idx}) -> 采样点 {end_sample_idx}")
    ...
    # 将 START_GRASS 与 END_GRASS 之间的所有点速度强制设为 -1000.0f
    for i in range(start_sample_idx, end_sample_idx + 1):
        output_target_speed[i] = -1000.0
