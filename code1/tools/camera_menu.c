...
static void CameraMenu_DrawStaticLayout(void)
{
    ...
    ips200_show_string(120, y + 7U * CAMERA_MENU_TEXT_Y_STEP, "M:");
                                  ...
                                  uint16 enable_mask)
{
    ...
    printf("[CAM1] task=%u mask=%u frame=%lu pvc=%u/%u conf=%.3f b2valid=%u src=%u mode=%u gate=%u bumpy=%u/%u dir=(%.2f,%.2f) cost=%lu\r\n",
           ...
           (unsigned long)CameraMenu_MaxFrameId(pvc->frame_id, bridge_v2_arbiter_get_frame_id(), bumpy->frame_id),
...
void CameraMenu_Update(void)
{
    ...
    ips200_show_uint(152, y + 0U * CAMERA_MENU_TEXT_Y_STEP,
                     CameraMenu_MaxFrameId(pvc->frame_id, bridge_v2_arbiter_get_frame_id(), bumpy->frame_id), 6);
