...
#define VISION_THREE_STAGE_POST_EXIT_DISTANCE_MM         (1000.0f) //先跑雷区要有这个，先跑单边桥改成0
#define VISION_THREE_STAGE_POST_EXIT_SPEED_SET           (-350.0f)

/* 像素阈值默认值（可在线调整） */
#define VISION_THREE_STAGE_JUMP1_BOTTOM_Y_DEFAULT        (28U) //第一级台阶底端小于该值时跳跃，增大阈值，车必须走得更近才跳
