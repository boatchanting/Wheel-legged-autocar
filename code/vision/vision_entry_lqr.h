...
/* ============================================================================
 * 模式 B：PD + 视觉扰动（默认，推荐主参数 K_E=11 / P=7 / D=1.0）
 ...
 *     - 航向抖动 / 保向段来回摆          → D +0.5~1（甜点 0~3.5，严禁 >0.5×P，
 *                                       否则 ψ_err'=−ω 的代数反馈把增益压缩 (1+D) 倍）
 *     - 仅高速振荡、低速正常             → 先 K_E −2，再考虑 P −1
 *   D 项实现：2ms 差分 + 一阶低通（PD_D_TAU_S），防 IMU 噪声放大。
 * ============================================================================ */
#define PD_K_E                  (11.0f)   /* 视觉扰动放大系数（对应原 k1=√Qy） */
#define PD_P_PSI                (7.0f)    /* 航向比例（对应原 k2，固定折中值） */
#define PD_D_PSI                (1.0f)    /* 航向微分阻尼（甜点 0~3.5） */
