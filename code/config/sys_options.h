...
#define SLIP_DETECTION_ENABLE 0U //【全局开关】1:开启基于侧向加速度的纯横向打滑检测  0:关闭（默认正常抓地权重）
#define GPS_FEEDFORWARD_ENABLE 1U //【全局开关】1:启用GPS硬件延时前馈补偿（推荐）  0:关闭前馈补偿
// ---------------- plan 配置 ----------------
#define GNSS_NAV 1 // 【全局开关】gps寻迹还是惯导寻迹，现阶段暂时还没联合(date0511)，联合后考虑去除该开关，1表示使用gnss寻迹，0表示不使用gnss寻迹，惯导开关常开
...
// ---------------- 航向基准配置 ----------------
#define ENABLE_DYNAMIC_HEADING 0 // 【发车角开关】1: 发车时自动获取当前地磁/GPS航向为基准；0: 使用下面预设的固定发车角
#define FIXED_BASE_YAW 305.0f    // 【固定基准发车角】发车时车头对准的方向(与正北夹角)，当不开启动态获取时使用此值
