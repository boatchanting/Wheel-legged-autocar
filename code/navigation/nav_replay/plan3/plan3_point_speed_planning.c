...
static float s_prev_speed_cmd = 0.0f;       // 用于速度斜率限制的静态变量
...
static float PlanDistanceSpeedAbs(float dist_mm, float stop_radius_mm)
{
    ...
    speed_abs = sqrtf(2.0f * NAV_PLAN3_SPEED_DECEL_CMD2_PER_MM * remain);
    speed_abs = Float_Constrain(speed_abs, 0.0f, fabsf(NAV_SPEED_FAST));

    if (speed_abs < fabsf(NAV_SPEED_SLOW))
    {
...
static float SpeedSlew(float raw_speed)
{
    ...
    float step_limit = NAV_PLAN3_SPEED_ACCEL_STEP;
    ...
    if ((raw_speed * s_prev_speed_cmd) < 0.0f)
    {
        step_limit = NAV_PLAN3_SPEED_CROSS_ZERO_STEP;
    }
    else if (fabsf(raw_speed) < fabsf(s_prev_speed_cmd))
    {
        step_limit = NAV_PLAN3_SPEED_DECEL_STEP;
    }
...
void NavReplay_Process(void)
{
    ...
    if (s_special_handoff_ticks > 0U)
    {
        ...
        if (g_target_idx >= nav_ram_data.point_count)
        {
            ...
            s_prev_speed_cmd = NAV_SPEED_STOP;
            err_degree = 0.0f;
            return;
        }
    ...
    else
    {
        ...
        // 如果角度偏差过大，则原地转向；否则利用速度规划减速逼近
        if (fabsf(NormalizeAngle(target_yaw - inertial_nav.relative_yaw)) > NAV_YAW_TOLERANCE)
        {
            target_speed_set = SpeedSlew(NAV_SPEED_STOP);
        }
        else
        {
            float speed_mag = PlanDistanceSpeedAbs(dist, 0.0f); // 目标是到达该点，所以停止距离设为 0
            target_speed_set = SpeedSlew(-speed_mag);
        }
    }
}
