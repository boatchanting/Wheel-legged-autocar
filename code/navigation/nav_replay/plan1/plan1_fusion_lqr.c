...
Plan1FusionPose_t plan1_fusion_pose = {0};
volatile uint8 plan1_fusion_beep_request = PLAN1_FUSION_BEEP_NONE;
...
typedef enum
{
    PLAN1_FUSION_ALIGN_STATE_WAIT_GPS = 0,
    PLAN1_FUSION_ALIGN_STATE_COLLECTING,
    PLAN1_FUSION_ALIGN_STATE_READY,
    PLAN1_FUSION_ALIGN_STATE_FAILED
} Plan1FusionAlignState_e;

static uint8 s_fusion_initialized = 0U;
static uint8 s_gps_aligned_to_path = 0U;
static uint8 s_has_last_gps = 0U;
static uint32 s_last_gnss_update_count = 0U;
static float s_last_inertial_x_mm = 0.0f;
static float s_last_inertial_y_mm = 0.0f;
static float s_last_gps_raw_x_mm = 0.0f;
static float s_last_gps_raw_y_mm = 0.0f;
static float s_gps_to_path_offset_x_mm = 0.0f;
static float s_gps_to_path_offset_y_mm = 0.0f;
static float s_gps_to_path_cos = 1.0f;
static float s_gps_to_path_sin = 0.0f;
static Plan1FusionAlignState_e s_align_state = PLAN1_FUSION_ALIGN_STATE_WAIT_GPS;
static uint16 s_align_ticks = 0U;
static uint8 s_align_has_yaw_candidate = 0U;
static float s_align_yaw_candidate_deg = 0.0f;
static float s_align_ins_start_x_mm = 0.0f;
static float s_align_ins_start_y_mm = 0.0f;
static float s_align_gps_start_x_mm = 0.0f;
static float s_align_gps_start_y_mm = 0.0f;
static uint8 s_wheel_unreliable_active = 0U;
static uint16 s_wheel_unreliable_hold_ticks = 0U;
static uint8 s_was_route_high_curvature = 0U;
static uint16 s_turn_entry_unreliable_ticks = 0U;
static uint8 s_gps_correction_beep_sent = 0U;
...
static void Plan1Fusion_ResetState(void);
static float NormalizeAngle(float angle);

static void Plan1Fusion_SetGpsToPathYaw(float yaw_deg)
{
    float yaw_rad = yaw_deg * LQR_DEG_TO_RAD;
    s_gps_to_path_cos = cosf(yaw_rad);
    s_gps_to_path_sin = sinf(yaw_rad);
}

static void Plan1Fusion_UpdateGpsRotation(void)
{
    Plan1Fusion_SetGpsToPathYaw(PLAN1_FUSION_GPS_TO_PATH_YAW_DEG);
}

static void Plan1Fusion_RequestBeep(uint8 request)
{
    if (plan1_fusion_beep_request == PLAN1_FUSION_BEEP_NONE)
    {
        plan1_fusion_beep_request = request;
    }
}

static void Plan1Fusion_RotateGpsToPath(float gps_x_mm, float gps_y_mm,
                                        float *path_x_mm, float *path_y_mm)
{
    *path_x_mm = s_gps_to_path_cos * gps_x_mm - s_gps_to_path_sin * gps_y_mm;
    *path_y_mm = s_gps_to_path_sin * gps_x_mm + s_gps_to_path_cos * gps_y_mm;
}

static float Plan1Fusion_CurrentX(void)
{
    return plan1_fusion_pose.x_mm;
}

static float Plan1Fusion_CurrentY(void)
{
    return plan1_fusion_pose.y_mm;
}

static float Plan1Fusion_ClampVector(float *x, float *y, float max_len)
{
    float len = sqrtf((*x) * (*x) + (*y) * (*y));

    if ((len > max_len) && (len > 1.0e-6f))
    {
...
static uint8 Plan1Fusion_IsRouteHighCurvature(void)
{
    float curvature_abs;

    if ((nav_ram_data.point_count == 0U) ||
        (g_target_idx >= nav_ram_data.point_count))
    {
        return 0U;
    }

    curvature_abs = fabsf(nav_ram_data.points[g_target_idx].curvature);
    return (uint8)(curvature_abs >= PLAN1_FUSION_UNRELIABLE_CURVATURE_TH);
}

static uint8 Plan1Fusion_IsWheelUnreliableTrigger(void)
{
    if (inertial_nav.slip_flag != 0U)
    {
        return 1U;
    }
    ...
    if (fabsf(inertial_nav.actual_yaw_rate) >= PLAN1_FUSION_UNRELIABLE_YAW_RATE_RAD_S)
    {
        return 1U;
    }
...
static void Plan1Fusion_UpdateWheelUnreliableState(void)
{
    uint8 route_high_curvature = Plan1Fusion_IsRouteHighCurvature();

    if ((route_high_curvature != 0U) && (s_was_route_high_curvature == 0U))
    {
        s_turn_entry_unreliable_ticks = PLAN1_FUSION_TURN_ENTRY_UNRELIABLE_TICKS;
    }
    s_was_route_high_curvature = route_high_curvature;

    if (Plan1Fusion_IsWheelUnreliableTrigger() != 0U)
    {
        s_wheel_unreliable_active = 1U;
        s_wheel_unreliable_hold_ticks = PLAN1_FUSION_UNRELIABLE_HOLD_TICKS;
        return;
    }

    if (s_turn_entry_unreliable_ticks > 0U)
    {
        s_turn_entry_unreliable_ticks--;
        s_wheel_unreliable_active = 1U;
        return;
    }

    if (s_wheel_unreliable_hold_ticks > 0U)
    {
        s_wheel_unreliable_hold_ticks--;
        s_wheel_unreliable_active = 1U;
    }
    else
    {
        s_wheel_unreliable_active = 0U;
    }
}

static uint8 Plan1Fusion_IsWheelUnreliable(void)
{
    return s_wheel_unreliable_active;
}

static void Plan1Fusion_ApplyPredictionDelta(float dx, float dy)
{
    if (Plan1Fusion_IsWheelUnreliable() != 0U)
    {
        dx *= PLAN1_FUSION_UNRELIABLE_PREDICT_SCALE;
        dy *= PLAN1_FUSION_UNRELIABLE_PREDICT_SCALE;
    }

    plan1_fusion_pose.x_mm += dx;
    plan1_fusion_pose.y_mm += dy;
}

static uint8 Plan1Fusion_IsAligning(void)
{
    return (uint8)((s_align_state == PLAN1_FUSION_ALIGN_STATE_WAIT_GPS) ||
                   (s_align_state == PLAN1_FUSION_ALIGN_STATE_COLLECTING));
}

static float Plan1Fusion_VectorYawDeg(float dx, float dy)
{
    return (float)(atan2f(dy, dx) * (180.0f / 3.1415926535f));
}

static void Plan1Fusion_FailAlignmentStop(void)
{
    s_align_state = PLAN1_FUSION_ALIGN_STATE_FAILED;
    s_gps_aligned_to_path = 0U;
    s_align_has_yaw_candidate = 0U;

    plan1_fusion_pose.gps_used = 0U;
    plan1_fusion_pose.gps_quality = (uint8)PLAN1_FUSION_GPS_QUALITY_INVALID;
    plan1_fusion_pose.fusion_mode = (uint8)PLAN1_FUSION_MODE_ALIGN_FAILED;
    Plan1Fusion_RequestBeep((uint8)PLAN1_FUSION_BEEP_ALIGN_FAILED);
...
static uint8 Plan1Fusion_UpdateAlignment(float gps_raw_x_mm,
                                         float gps_raw_y_mm,
                                         float gps_frame_step_mm)
{
    float ins_dx;
    float ins_dy;
    float gps_dx;
    ...
    if (s_align_state == PLAN1_FUSION_ALIGN_STATE_READY)
    {
        return 1U;
    }

    if (s_align_state == PLAN1_FUSION_ALIGN_STATE_FAILED)
    {
        plan1_fusion_pose.fusion_mode = (uint8)PLAN1_FUSION_MODE_ALIGN_FAILED;
        return 0U;
    }

    plan1_fusion_pose.gps_quality = (uint8)PLAN1_FUSION_GPS_QUALITY_WEAK;
    plan1_fusion_pose.fusion_mode = (uint8)PLAN1_FUSION_MODE_ALIGNING;

    if ((s_has_last_gps != 0U) &&
        (gps_frame_step_mm > PLAN1_FUSION_ALIGN_MAX_FRAME_STEP_MM))
    {
        Plan1Fusion_FailAlignmentStop();
        return 0U;
    }
    ...
    if (s_align_state == PLAN1_FUSION_ALIGN_STATE_WAIT_GPS)
    {
        s_align_gps_start_x_mm = gps_raw_x_mm;
        s_align_gps_start_y_mm = gps_raw_y_mm;
        s_align_state = PLAN1_FUSION_ALIGN_STATE_COLLECTING;
        return 0U;
    }

    ins_dx = plan1_fusion_pose.x_mm - s_align_ins_start_x_mm;
    ins_dy = plan1_fusion_pose.y_mm - s_align_ins_start_y_mm;
    gps_dx = gps_raw_x_mm - s_align_gps_start_x_mm;
    gps_dy = gps_raw_y_mm - s_align_gps_start_y_mm;
    ins_dist_mm = sqrtf(ins_dx * ins_dx + ins_dy * ins_dy);
    gps_dist_mm = sqrtf(gps_dx * gps_dx + gps_dy * gps_dy);
    plan1_fusion_pose.gps_residual_mm = fabsf(ins_dist_mm - gps_dist_mm);

    if ((ins_dist_mm < PLAN1_FUSION_ALIGN_MIN_INS_DIST_MM) ||
        (gps_dist_mm < PLAN1_FUSION_ALIGN_MIN_GPS_DIST_MM))
    {
        return 0U;
    }

    ins_yaw_deg = Plan1Fusion_VectorYawDeg(ins_dx, ins_dy);
    gps_yaw_deg = Plan1Fusion_VectorYawDeg(gps_dx, gps_dy);
    auto_yaw_deg = NormalizeAngle(ins_yaw_deg - gps_yaw_deg);

    if (s_align_has_yaw_candidate == 0U)
    {
        s_align_yaw_candidate_deg = auto_yaw_deg;
        s_align_has_yaw_candidate = 1U;
        return 0U;
    }

    {
        float yaw_delta_deg = NormalizeAngle(auto_yaw_deg - s_align_yaw_candidate_deg);
        float align_yaw_deg;
        float gps_rot_x_mm;
        float gps_rot_y_mm;

        if (fabsf(yaw_delta_deg) > PLAN1_FUSION_ALIGN_MAX_DIR_ERR_DEG)
        {
            Plan1Fusion_FailAlignmentStop();
            return 0U;
        }

        align_yaw_deg = NormalizeAngle(s_align_yaw_candidate_deg + yaw_delta_deg * 0.5f);
        Plan1Fusion_SetGpsToPathYaw(align_yaw_deg);
        Plan1Fusion_RotateGpsToPath(gps_raw_x_mm, gps_raw_y_mm,
                                    &gps_rot_x_mm, &gps_rot_y_mm);

        s_gps_to_path_offset_x_mm = plan1_fusion_pose.x_mm - gps_rot_x_mm;
        s_gps_to_path_offset_y_mm = plan1_fusion_pose.y_mm - gps_rot_y_mm;
        s_align_state = PLAN1_FUSION_ALIGN_STATE_READY;
        s_gps_aligned_to_path = 1U;
        s_align_has_yaw_candidate = 0U;
        plan1_fusion_pose.gps_residual_mm = 0.0f;
        plan1_fusion_pose.fusion_mode = (uint8)PLAN1_FUSION_MODE_INS_ONLY;
        Plan1Fusion_RequestBeep((uint8)PLAN1_FUSION_BEEP_ALIGN_READY);
        return 1U;
    }
}
...
static void Plan1Fusion_ResetState(void)
{
    ...
    Plan1Fusion_UpdateGpsRotation();

    plan1_fusion_pose.x_mm = inertial_nav.x;
    plan1_fusion_pose.y_mm = inertial_nav.y;
    plan1_fusion_pose.yaw_deg = inertial_nav.relative_yaw;
    plan1_fusion_pose.vx_mm_s = inertial_nav.vx_body;
    plan1_fusion_pose.vy_mm_s = inertial_nav.vy_body;
    plan1_fusion_pose.gps_used = 0U;
    plan1_fusion_pose.gps_quality = (uint8)PLAN1_FUSION_GPS_QUALITY_INVALID;
    plan1_fusion_pose.gps_residual_mm = 0.0f;
    plan1_fusion_pose.fusion_mode = (uint8)PLAN1_FUSION_MODE_ALIGNING;

    s_last_inertial_x_mm = inertial_nav.x;
    s_last_inertial_y_mm = inertial_nav.y;
    s_last_gnss_update_count = gnss_trans.update_count;
    s_last_gps_raw_x_mm = 0.0f;
    s_last_gps_raw_y_mm = 0.0f;
    s_gps_to_path_offset_x_mm = 0.0f;
    s_gps_to_path_offset_y_mm = 0.0f;
    s_has_last_gps = 0U;
    s_gps_aligned_to_path = 0U;
    s_fusion_initialized = 1U;
    s_align_state = PLAN1_FUSION_ALIGN_STATE_WAIT_GPS;
    s_align_ticks = 0U;
    s_align_has_yaw_candidate = 0U;
    s_align_yaw_candidate_deg = 0.0f;
    s_align_ins_start_x_mm = inertial_nav.x;
    s_align_ins_start_y_mm = inertial_nav.y;
    s_align_gps_start_x_mm = 0.0f;
    s_align_gps_start_y_mm = 0.0f;
    s_wheel_unreliable_active = 0U;
    s_wheel_unreliable_hold_ticks = 0U;
    s_was_route_high_curvature = 0U;
    s_turn_entry_unreliable_ticks = 0U;
    s_gps_correction_beep_sent = 0U;
...
void Plan1FusionLqr_UpdateFusion10ms(void)
{
    ...
    if (s_fusion_initialized == 0U)
    {
        Plan1Fusion_ResetState();
        return;
    }

    dx = inertial_nav.x - s_last_inertial_x_mm;
    dy = inertial_nav.y - s_last_inertial_y_mm;
    s_last_inertial_x_mm = inertial_nav.x;
    s_last_inertial_y_mm = inertial_nav.y;
    Plan1Fusion_UpdateWheelUnreliableState();

    Plan1Fusion_ApplyPredictionDelta(dx, dy);
    plan1_fusion_pose.yaw_deg = inertial_nav.relative_yaw;
    plan1_fusion_pose.vx_mm_s = inertial_nav.vx_body;
    plan1_fusion_pose.vy_mm_s = inertial_nav.vy_body;

    if ((g_replay_state == REPLAY_RUNNING) && (Plan1Fusion_IsAligning() != 0U))
    {
        if (s_align_ticks < 0xFFFFU)
        {
            s_align_ticks++;
        }

        plan1_fusion_pose.fusion_mode = (uint8)PLAN1_FUSION_MODE_ALIGNING;
        if (s_align_ticks >= PLAN1_FUSION_ALIGN_TIMEOUT_TICKS)
        {
            Plan1Fusion_FailAlignmentStop();
        }
    }
    else if (s_align_state == PLAN1_FUSION_ALIGN_STATE_FAILED)
    {
        plan1_fusion_pose.fusion_mode = (uint8)PLAN1_FUSION_MODE_ALIGN_FAILED;
    }
    else if ((s_align_state != PLAN1_FUSION_ALIGN_STATE_READY) ||
             (s_gps_aligned_to_path == 0U))
    {
        plan1_fusion_pose.fusion_mode = (uint8)PLAN1_FUSION_MODE_ALIGNING;
    }
    else if ((Plan1Fusion_IsWheelUnreliable() != 0U) &&
             (plan1_fusion_pose.gps_used == 0U))
    {
        plan1_fusion_pose.fusion_mode = (uint8)PLAN1_FUSION_MODE_WHEEL_UNRELIABLE;
    }
    else if (plan1_fusion_pose.gps_used == 0U)
    {
        plan1_fusion_pose.fusion_mode = (uint8)PLAN1_FUSION_MODE_INS_ONLY;
    }
    plan1_fusion_pose.gps_used = 0U;
...
void Plan1FusionLqr_CorrectWithGnss(void)
{
    ...
    float residual_y_mm;
    float residual_mm;
    float gps_frame_step_mm = 0.0f;
    float speed_mm_s;
    float gain;
    float max_correct_step;
    float correct_x_mm;
    float correct_y_mm;
    uint8 weak_gps = 0U;
    uint8 wheel_unreliable;
    float reject_residual_mm;
    ...
    plan1_fusion_pose.gps_used = 0U;
    wheel_unreliable = Plan1Fusion_IsWheelUnreliable();
    ...
    if ((gnss_trans.is_valid == 0U) ||
        (gnss_trans.is_origin_set == 0U) ||
        (gnss.state != 1U) ||
        (gnss.satellite_used < ((Plan1Fusion_IsAligning() != 0U) ?
                                PLAN1_FUSION_ALIGN_MIN_SAT_USED :
                                PLAN1_FUSION_MIN_SAT_USED)))
    {
        plan1_fusion_pose.gps_quality = (uint8)PLAN1_FUSION_GPS_QUALITY_INVALID;
        plan1_fusion_pose.fusion_mode = (Plan1Fusion_IsAligning() != 0U) ?
                                        (uint8)PLAN1_FUSION_MODE_ALIGNING :
                                        (uint8)PLAN1_FUSION_MODE_INS_ONLY;
        return;
    }

    if (gnss_trans.update_count == s_last_gnss_update_count)
    {
        return;
    }
    s_last_gnss_update_count = gnss_trans.update_count;

    gps_raw_x_mm = gnss_trans.x * 1000.0f;
    gps_raw_y_mm = gnss_trans.y * 1000.0f;

    if (s_has_last_gps != 0U)
    {
        float gps_dx = gps_raw_x_mm - s_last_gps_raw_x_mm;
        float gps_dy = gps_raw_y_mm - s_last_gps_raw_y_mm;
        gps_frame_step_mm = sqrtf(gps_dx * gps_dx + gps_dy * gps_dy);
    }

    if (Plan1Fusion_IsAligning() != 0U)
    {
        (void)Plan1Fusion_UpdateAlignment(gps_raw_x_mm, gps_raw_y_mm, gps_frame_step_mm);
        s_last_gps_raw_x_mm = gps_raw_x_mm;
        s_last_gps_raw_y_mm = gps_raw_y_mm;
        s_has_last_gps = 1U;
        return;
    }
    ...
    gps_path_x_mm = gps_rot_x_mm + s_gps_to_path_offset_x_mm;
    gps_path_y_mm = gps_rot_y_mm + s_gps_to_path_offset_y_mm;
    residual_x_mm = gps_path_x_mm - plan1_fusion_pose.x_mm;
    residual_y_mm = gps_path_y_mm - plan1_fusion_pose.y_mm;
    residual_mm = sqrtf(residual_x_mm * residual_x_mm + residual_y_mm * residual_y_mm);
    plan1_fusion_pose.gps_residual_mm = residual_mm;
    reject_residual_mm = (wheel_unreliable != 0U) ?
                         PLAN1_FUSION_UNRELIABLE_REJECT_RESIDUAL_MM :
                         PLAN1_FUSION_GPS_REJECT_RESIDUAL_MM;

    if ((residual_mm > reject_residual_mm) ||
        (gps_frame_step_mm > PLAN1_FUSION_GPS_MAX_FRAME_STEP_MM))
    {
        plan1_fusion_pose.gps_quality = (uint8)PLAN1_FUSION_GPS_QUALITY_INVALID;
        plan1_fusion_pose.fusion_mode = (uint8)PLAN1_FUSION_MODE_GPS_REJECT;
        return;
    }

    speed_mm_s = sqrtf(inertial_nav.vx_body * inertial_nav.vx_body +
                       inertial_nav.vy_body * inertial_nav.vy_body);

    if ((gnss.satellite_used < PLAN1_FUSION_GOOD_SAT_USED) ||
        (residual_mm > PLAN1_FUSION_GPS_WEAK_RESIDUAL_MM) ||
        (speed_mm_s > PLAN1_FUSION_MAX_SPEED_MM_S))
    {
        weak_gps = 1U;
    }

    if (wheel_unreliable != 0U)
    {
        gain = PLAN1_FUSION_UNRELIABLE_GPS_GAIN;
        max_correct_step = PLAN1_FUSION_UNRELIABLE_GPS_STEP_MM;
        plan1_fusion_pose.gps_quality = (gnss.satellite_used >= PLAN1_FUSION_GOOD_SAT_USED) ?
                                        (uint8)PLAN1_FUSION_GPS_QUALITY_NORMAL :
                                        (uint8)PLAN1_FUSION_GPS_QUALITY_WEAK;
        plan1_fusion_pose.fusion_mode = (uint8)PLAN1_FUSION_MODE_WHEEL_UNRELIABLE;
    }
    else if (weak_gps != 0U)
    {
        gain = PLAN1_FUSION_GPS_WEAK_GAIN;
        max_correct_step = PLAN1_FUSION_GPS_WEAK_STEP_MM;
        plan1_fusion_pose.gps_quality = (uint8)PLAN1_FUSION_GPS_QUALITY_WEAK;
        plan1_fusion_pose.fusion_mode = (uint8)PLAN1_FUSION_MODE_GPS_WEAK;
    }
    else
    {
        gain = PLAN1_FUSION_GPS_NORMAL_GAIN;
        max_correct_step = PLAN1_FUSION_GPS_NORMAL_STEP_MM;
        plan1_fusion_pose.gps_quality = (uint8)PLAN1_FUSION_GPS_QUALITY_NORMAL;
        plan1_fusion_pose.fusion_mode = (uint8)PLAN1_FUSION_MODE_GPS_NORMAL;
    }

    correct_x_mm = residual_x_mm * gain;
    correct_y_mm = residual_y_mm * gain;
    (void)Plan1Fusion_ClampVector(&correct_x_mm, &correct_y_mm, max_correct_step);

    plan1_fusion_pose.x_mm += correct_x_mm;
    plan1_fusion_pose.y_mm += correct_y_mm;
    plan1_fusion_pose.gps_used = 1U;
    if (s_gps_correction_beep_sent == 0U)
    {
        s_gps_correction_beep_sent = 1U;
        Plan1Fusion_RequestBeep((uint8)PLAN1_FUSION_BEEP_GPS_CORRECT_START);
    }
}
...
static uint8 NavReplay_FastUTurn_ShouldTrigger(uint16 base_idx)
{
    ...
    dist_to_action = CalcDistance(Plan1Fusion_CurrentX(), Plan1Fusion_CurrentY(),
...
void NavReplay_Start(void)
{
    ...
    {
#if DEBUG_LOG_ENABLE
        printf("[Fusion-LQR] RAM is empty, cannot start replay.\r\n");
#endif
        return;
    }
...
#if DEBUG_LOG_ENABLE
    printf("[Fusion-LQR] Replay START. Plan: %d, Total Points: %d\r\n",
...
#if DEBUG_LOG_ENABLE
    printf("[Fusion-LQR] Replay STOPPED.\r\n");
#endif
}
...
static int Find_Closest_Point_Index_Strict(int current_idx, int search_range, uint8 is_recovering)
{
    ...
    {
        float d_sq = CalcDistanceSq(Plan1Fusion_CurrentX(), Plan1Fusion_CurrentY(),
...
static void NavReplay_BuildReference(uint16 base_idx, uint16 stop_idx, LqrReference_t *ref)
{
    ...
    {
        float car_dx = Plan1Fusion_CurrentX() - base->x;
        float car_dy = Plan1Fusion_CurrentY() - base->y;
...
{
    float dx = Plan1Fusion_CurrentX() - ref->x;
    float dy = Plan1Fusion_CurrentY() - ref->y;
...
void NavReplay_Process(void)
{
...
#if DEBUG_LOG_ENABLE
        printf("[Fusion-LQR] Start heading aligned, launch pose reset.\r\n");
#endif
        return;
    }
#endif
    ...
    dist_to_stop = CalcDistance(Plan1Fusion_CurrentX(), Plan1Fusion_CurrentY(),
