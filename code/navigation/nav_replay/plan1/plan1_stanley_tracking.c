...
#if (CURRENT_NAV_PLAN == 1) && (NAV_PLAN1_METHOD == PLAN1_STANLEY_TRACKING)
...
void NavReplay_Process(void)
{
    ...
    else
    {
        ...
        if (next_idx > last_idx) next_idx = last_idx;
        ...
        float dx_path = x2 - x1;
        float dy_path = y2 - y1;
        float dx_car = inertial_nav.x - x1;
        float dy_car = inertial_nav.y - y1;
        ...
        raw_err_degree = NormalizeAngle(heading_error + cte_correction_rad * 57.29578f);

        // 曲率旁路判定：使用 stop_idx 防止看穿停车线
        int bypass_idx = current_idx + 15;
        if (bypass_idx > stop_idx) bypass_idx = stop_idx;
        ...
        if (fabsf(bypass_kappa) > KAPPA_CURVE_BYPASS_THRESH)
        {
            ...
            s_prev_err_degree = err_degree; // 保存历史滤波状态
        }
        else
        {
    ...
    /* 阶段6.5：曲率前馈 — 入弯前主动转向，减少反馈滞后 */
    {
        ...
        if (ff_idx > stop_idx) ff_idx = stop_idx; // 前馈曲率不越过 stop_idx
