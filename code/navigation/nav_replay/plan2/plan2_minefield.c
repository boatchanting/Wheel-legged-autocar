...
void NavReplay_Process(void){if(g_replay_state!=REPLAY_RUNNING||nav_ram_data.point_count==0)return; if(g_target_idx>=nav_ram_data.point_count){g_replay_state=REPLAY_FINISHED;target_speed_set=0;err_degree=0;return;} float tx=nav_ram_data.points[g_target_idx].x,ty=nav_ram_data.points[g_target_idx].y; float d=CalcDistance(inertial_nav.x,inertial_nav.y,tx,ty); float yaw=-atan2f(ty-inertial_nav.y,-(tx-inertial_nav.x))*57.29578f; err_degree=NormalizeAngle(yaw-inertial_nav.relative_yaw); target_speed_set=nav_ram_data.points[g_target_idx].target_speed; g_current_point_type=nav_ram_data.points[g_target_idx].point_type; if(g_current_point_type==NAV_POINT_CIRCLE&&d<20.0f){minefield_flag=1;g_special_action_trigger=1;g_target_idx++;return;} if(d<20.0f) g_target_idx++;}
#endif
