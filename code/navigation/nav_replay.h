...
#define GPS_NAV_DIST_ARRIVE               500.0f   // 仅用于终点停车的判定距离 (0.5�?
#define GPS_NAV_DIST_NEAR                 2200.0f  // 速度控制的远近临界�?
...
#define NAV_DIST_ARRIVE         20.0f   // 到达判定阈�?
#define NAV_YAW_TOLERANCE        1.0f    //转向阈值，先转再走
#define NAV_START_HEADING_TOLERANCE 0.3f // 复刻起步前，绝对航向对齐阈�?�?，这个需要使用地磁计航向角，暂时不用调【优化点�?

// 速度设定 (负数为前进，数值对�?motor rpm �?pwm �?
#define NAV_SPEED_FAST          (-400.0f) // 高速行驶速度
#define NAV_SPEED_SLOW          (-200.0f)  // 低速逼近速度 (-60 约等�?20cm/s) 
...
// --- 1. 纯追�?(Pure Pursuit) 导航参数 ---
#define PP_LD_MIN_CURVE        500.0f   // 弯道最小前�?(mm)。越小越贴线，但容易抖动。要求精�?5mm建议不低�?00�?
#define PP_LD_MIN_STRAIGHT     1.2f     // 直道前瞻倍率。针�?m大点距，建议设为当前点距�?.1-1.5倍�?
...
#define SPD_CURVE_EXPONENT     2.5f     // 曲率减速指数�?.0为线性，2.0为平方律。越大则轻微弯道速度越快�?
#define SPD_ANGLE_PENALTY      0.15f     // 转向角度惩罚权重 (0-1)。值越小，纠偏时减速越少，动力更足�?
#define SPD_ANGLE_TOLERANCE    60.0f    // 转向角度容忍门槛 (�?。角度偏差在此范围内不触发剧烈减速�?
...
#define SLEW_RATE_ANGLE        35.0f    // 单次周期最大转角变�?(�?。防止电�?舵机瞬间猛打�?
#endif

#if CURRENT_NAV_PLAN == 2 //科目二参�?
// ========================= 控制参数宏定�?=========================
// 距离阈�?(单位: mm)
#define NAV_DIST_FAR            700.0f  // 远距离界限，全�?
#define NAV_DIST_NEAR           500.0f  // 近距离界限，开始最低�?

#define NAV_DIST_ARRIVE         20.0f   // 到达判定阈�?
#define NAV_YAW_TOLERANCE        1.0f    //转向阈值，先转再走
#define NAV_START_HEADING_TOLERANCE 0.3f // 复刻起步前，绝对航向对齐阈�?�?

// 速度设定 (负数为前进，数值对�?motor rpm �?pwm �?
#define NAV_SPEED_FAST          (-300.0f) // 高速行驶速度
#define NAV_SPEED_SLOW          (-100.0f)  // 低速逼近速度 (-60 约等�?20cm/s) 
...
#define SPD_CURVE_EXPONENT     3.5f     // 曲率减速指数�?.0为线性，2.0为平方律。越大则轻微弯道速度越快�?
...
#if CURRENT_NAV_PLAN == 3 //科目三参�?
// ========================= 控制参数宏定�?=========================
// 距离阈�?(单位: mm)
#define NAV_DIST_FAR            500.0f  // 远距离界限，全�?
#define NAV_DIST_NEAR           150.0f  // 近距离界限，开始最低�?
...
#endif
// ========================= 全局控制变量声明 =========================
// 这些变量由外部定�?(通常�?control.c �?main.c)，此处引�?
...
// 暴露给外部的状�?
extern NavReplayState_e g_replay_state;         // 当前复现状�?
extern uint8 g_current_point_type;              // 当前正在前往/到达的点的类�?
extern uint8 g_special_action_trigger;          // 特殊动作触发标志 (1: 到达特殊点，请执行动�?
...
/**
 * @brief  惯性导航复现控制周期函�?
 * @note   建议放在 10ms �?20ms 定时器中断或主循环中调用
 *         它会根据当前 inertial_nav 坐标计算 target_speed_set �?err_degree
 */
