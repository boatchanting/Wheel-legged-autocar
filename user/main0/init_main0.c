...
#if CAMERA_ONLY_MODE
    /* =====================================================================
     * 摄像头直传模式（破坏性裁剪）：0核不再做任何控制/导航/任务初始化。
     * 只保留：时钟 + 调试串口。
     * 摄像头读取 与 WiFi 图像/示波器转发 全部由 1核 (main_cm7_1.c) 完成。
     * 这里仍调用 VisionIpc_Core0_Init() 向 1核 共享内存写入一条"空指令"，
     * 确保 1核 的 PVC/单边桥/颠簸 视觉算法保持关闭（防止残留旧命令被误触发）。
     * ===================================================================== */
    VisionIpc_Core0_Init();   // 仅写共享内存空指令(enable_mask=0)，无硬件初始化
    return;
#else
    // 此处编写用户代码 例如外设初始化代码等
