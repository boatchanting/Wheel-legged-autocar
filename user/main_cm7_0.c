...
// =================================================================================
// ★★★ PID调试阶段选择 (修改这里来切换调试模式) ★★★
// 0: 正常运行模式 (所有环生效)
// 1: 仅调试【角速度环】 (最内环) -> 车身有阻尼感
// 2: 仅调试【角度环】   (中环)   -> 车身能直立
// 3: 仅调试【速度环】   (最外环) -> 车身能定点/受控
// =================================================================================
#define TUNING_STAGE  1 // 修改这里来切换调试模式  
// PID控制中间变量
...
int main(void)
{
    ...
    if(TUNING_STAGE == 1) {
        // 阶段1：只调角速度环
        pid_speed.kp = 0; pid_speed.ki = 0;
        pid_angle.kp = 0; pid_angle.kd = 0;
        // 在这里修改参数调试：
        pid_gyro.kp  = 2.5; // <--- 调节这个
        pid_gyro.kd  = 0.8; // <--- 调节这个
    }
    else if(TUNING_STAGE == 2) {
        // 阶段2：调角度环 (角速度环已固定)
        pid_speed.kp = 0; pid_speed.ki = 0;
        // 保持内环参数
        pid_gyro.kp  = 2.5; pid_gyro.kd  = 0.8; 
        // 在这里修改参数调试：
        pid_angle.kp = 45.0; // <--- 调节这个 (硬度)
        pid_angle.kd = 1.5;  // <--- 调节这个 (阻尼)
    }
    else if(TUNING_STAGE == 3) {
        // 阶段3：调速度环 (内两环已固定)
        pid_gyro.kp  = 2.5;  pid_gyro.kd  = 0.8;
        pid_angle.kp = 45.0; pid_angle.kd = 1.5;
        // 在这里修改参数调试：
        pid_speed.kp = 0.05;  // <--- 调节这个
        pid_speed.ki = 0.002; // <--- 调节这个
    }
